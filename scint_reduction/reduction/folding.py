...
def fold(foldtype, fn, obsfile, tstart, polyco, dtype, Tint, tbin, nchan, ngate, size, dedisperse, pulsedetect, deconvolve):
    ...
    **obs: parameters read from obs.conf
    ...
    for i in range(Tstep):
        ...
        if dedisperse == 'coherent':
            ...
            if deconvolve:
                kernel = np.load('ts2014-06-13T07:00:02.13597.npy')
                kernel = kernel[16450:16450+2048,:4]
                decon = np.zeros_like(data)
                decon[:2048] = kernel
                data = np.fft.rfft(data, axis=0)
                decon = np.fft.rfft(decon, axis=0)
                data /= ( decon / np.abs(decon) )
